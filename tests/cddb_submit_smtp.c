...
{
  printf("usage: %s [-d device] [-e email_address] [-s submit_address] [server[:port]]\n",name);
}
...
int main(int argc,char **argv)
{
  ...
  char submitaddr[BUFSIZ]="user@localhost";
  ...
  for(i=1;i<argc;i++)
  {
    ...
    else if(strcmp(argv[i],"-s")==0)
    {
      ...
      strncpy(submitaddr,argv[i],BUFSIZ);
    }
    else if(argv[i][0]!='-')
    {
  ...
  cddb_free_cddb_host(&host);
  cddb_init_cddb_host(&host);
  host.host_server.server_name=strdup(hostname);
  ...
  *ver++='\0';
  ...
  cddb_submit(&host,NULL,&hello,&disc,&data,"CDDB submission test",emailaddr,submitaddr);
