 ...
 * @param cd_desc the handle to the cd device contained the cd from which to calculate the CDDB query string.  
 ...
 */
int cddb_read_reply(cdsock_t sock,char *message,int len);
{
 ...
 * @param query a character string specifying the CDDB id and disk track info required for a CDDB query.  
 * @param sock handle to the socket connected to the CDDB server.  
...
int cddb_query(const char* query,cdsock_t sock,int mode,struct cddb_query *query,...);
{
  ...
  {
    http_string=va_arg(arglist,char *);
    snprintf(outtemp,sizeof(outtemp),"cddb+query+%s",query);
